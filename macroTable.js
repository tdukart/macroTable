		...
		//more normal situations
			...
			if(direction > 0) { 
				...
				var finalDomRowWindow = tableData.length - rowBuffer - rowBuffer - displayRowWindow, //the final row window render starts at this row
					//remainingRows = Math.max(tableData.length - currentRow, $tableRows.filter(':gt('+(currentDomRow - 1)+')').length),
					remainingDomRows = $tableRows.filter(':gt('+(currentDomRow - 1)+')').length;

				var isInFinalDomWindow = currentRow > finalDomRowWindow,
					//moreRowRenderingNeeded = tableData.length - currentRow + 1 >= remainingDomRows && remainingDomRows == maxTotalDomRows - rowBuffer;//remainingRows - rowBuffer - displayRowWindow > 0;
				...
				//if(currentDomRow >= triggerDownDomRow && currentRow <= tableData.length - rowBuffer/* - displayRowWindow*/) { 	//if scrolling fast, this window can be skipped near the bottom, 
																																//and the remaining rows won't ever be rendered, so making window larger by not subtracting displayRowWindow
